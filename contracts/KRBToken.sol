    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @dev Mapping of rewarded VCTypes.VerifiableCredentials. Key is a hash of the vc data
    ...
    event Issued(uint256 uuid, VCTypes.VerifiableCredential vc);
    ...
    function initialize(
        ...
        __KRBToken_init(name, symbol, version);
    }
    ...
    function __KRBToken_init(
        ...
        __KRBToken_init_unchained(name, symbol, version);
    }
    ...
    * @notice Grants `DEFAULT_ADMIN_ROLE` and `GOVERN_ROLE` to the
    ...
    function __KRBToken_init_unchained(
        string memory,
        string memory,
        string memory
    ) internal onlyInitializing {
     ...
     *
     */
    function getUuid(VCTypes.VerifiableCredential memory vc)
        public
        pure
        returns (uint256)
    {
        return uint256(VCTypes.getVerifiableCredential(vc));
    }
    ...
    function registerVC(
        ...
        //Mint rewards
        uint256 _reward = VCTypes.getReward(_stake, vc.credentialSubject.trust);
     ...
     *
     */
    function expiredVC(VCTypes.VerifiableCredential memory vc)
        external
        returns (bool)
    {
        uint256 uuid = getUuid(vc);
    ...
    function disputeVCByGovern(
        VCTypes.VerifiableCredential memory vc,
        VCTypes.VerifiableCredential memory disputeVC
        ...
        uint256 _disputeStake = disputeVC.credentialSubject.stake *
            10**decimals();
        uint256 _disputeReward = VCTypes.getReward(
            _disputeStake,
